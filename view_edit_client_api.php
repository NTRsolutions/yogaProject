...
if(isset($_POST["c_ID"]) ){
    ...
    if ($result->num_rows > 0) {
        ...
        array_push($client,array('c_ID'=>$row['c_ID'],'c_name'=>$row['c_name'],'c_surname'=>$row['c_surname'],'gender'=>$row['gender'],'DOB'=>$row['DOB'],'Anniversary'=>$row['Anniversary'],'Age'=>$row['Age'],'address'=>$row['address'],'contact'=>$row['contact'],'fees'=>$row['fees'],'Register_ID'=>$row['Register_ID'],'Lead_By'=>$row['Lead_By'],'photo'=>$row['photo'],'Comments'=>$row['Comments'],'status_payment'=>$row['status_payment'],'batch_name'=>$row1['batch_name']));
